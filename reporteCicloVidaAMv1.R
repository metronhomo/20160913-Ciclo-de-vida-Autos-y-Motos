...
funcionLlenaBloque<-function(bloque=2,nBloques=8,atributos=nombresItems,matriz=Maxdiff){
...
#Selecciono las columnas que van en el max diff
MaxDiff<- datos %>%
  filter(datos$P10=="Moto / Motocicleta" & datos$P18=="Si") %>%
  select(contains("MOTO_MAS"),-contains("AMBOS")) 
...
###Selecciono el nombre de los atributos
nombresItems<-unique(str_sub(names(MaxDiff),18,-1))
atributos4<-unique(str_sub(names(MaxDiff),18,-1))
##Asigno nuevos nobres a las variables, del tipo BiVk  #donde la i es el numero de bloque y la k el numero de atributo
MaxDiff<- nombresMaxDiff(MaxDiff,5,nombresItems,18)
names(MaxDiff)
...
resultados<-resultados %>%
  dplyr::arrange(logit)
resultados
write.csv(resultados,'RankeoMaxDiff4.csv')
valores<-c(0,logitModel$coefficients)
resultadoFinal<-cbind(valores,atributos4)
resultadoFinal
write.csv(resultadoFinal,'resultado final max diff4.csv')
...
resultados<-resultados %>%
  dplyr::arrange(logit)
resultados
write.csv(resultados,'RankeoMaxDiff5.csv')
valores<-c(0,logitModel$coefficients)
resultadoFinal<-cbind(valores,atributos5)
resultadoFinal
write.csv(resultadoFinal,'resultado final max diff5.csv')
